...
## 补充：新增显示指标列时主表与统计行顺序分叉

### 新增现象

- 在项目汇总等报表中，点击某个统计字段让它开始显示后，主表里的新列会按 ProTable 默认策略插入到靠前位置
- 统计行则因为内部把“无 `order` 的新列”兜底追加到末尾，导致同一个字段在主表和统计行的位置不一致

### 根因

- 新增显示的指标列没有立即拿到受控 `columnsStateMap.order`
- `ProTable` 主表渲染与统计行顺序推导在“缺失 `order`”场景下采用了不同默认策略
- 同时指标列生成顺序如果仍按 `metricOptions` 固定顺序，而不是按当前 `metrics` 选中顺序，会进一步放大“新列插队”现象

### 解决方式

- 在 `UniversalReportTable` 内部为当前激活列构造一份“有效列状态”，给缺失 `order` 的列补全顺序值
- 新增显示的指标列统一按当前布局追加到末尾，并将该顺序同时提供给 `ProTable` 和统计行
- 指标列基础顺序改为跟随当前 `metrics` 数组，而不是固定使用 `metricOptions` 的声明顺序

### 影响范围

- 所有使用 `UniversalReportTable`，并允许动态勾选指标列且开启统计行的报表页
