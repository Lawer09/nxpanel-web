...
项目管理页导入数据时，存在三类实际输入特征：
- 同一个 CSV 列需要映射到多个导入字段
- 粘贴文本中部分记录使用多个空格而不是换行分隔字段
- 粘贴记录缺少项目名称、负责人、包名等辅助字段，但仍希望继续导入

### 问题原因

- 原 CSV 映射对同一表头做了互斥禁用，无法表达“一列多用”
- 粘贴解析只按换行切分字段，无法兼容多空格分隔的行内字段
- 粘贴导入把部分缺失信息按硬错误处理，导致本可接受的记录被拦截

### 解决方式

- CSV 字段映射放开同一表头的重复选择，仅在下拉中展示“已选 N 次”计数提示
- 粘贴解析增加“多个空格分隔”拆分，但避开 URL 和 JSON 内容
- 缺少项目名称时默认回填为项目代号；缺少负责人、包名等信息降级为 warning，在预览中提示但仍允许导入

### 影响范围

项目管理页导入弹窗的 CSV 映射与粘贴文本模式。
...
项目管理页粘贴导入时，用户直接从表格软件复制一整行项目数据，内容表现为制表符分隔，并且某些单元格内部还会包含换行。

### 问题原因

原粘贴导入只支持“按项目代号起始行切块”的自由文本格式，无法识别 `\t` 分隔的一整行表格文本，因此会提示“未识别到可导入的项目记录”。
...
- 支持单元格内部换行续接
- 按固定列顺序映射到项目字段
- 若命中 TSV 分支，则不再依赖“项目代号起始行 + 下一行特征”规则
...
项目管理页导入预览中，部分字段会出现长文本或多行内容，例如广告位配置、Firebase 备注、补充说明等，导致表格单行被撑高，影响横向扫读效率。

### 问题原因

导入预览表中的业务字段列直接输出原始文本，没有做单行收敛；当字段值包含换行或长串文本时，Table 单元格会按内容自动换行。

### 解决方式

预览表的业务字段列统一改为单行省略展示，并通过 tooltip 悬浮显示完整内容；tooltip 保留原始换行，方便查看长文本明细。

### 影响范围

项目管理页导入弹窗的 CSV / 粘贴文本预览表。
