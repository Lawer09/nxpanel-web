...
import { getProjectOptions, getTeamOptions } from "@/services/ads-console/orgOptions";
import {
  DownloadOutlined,
...
function toCSV(rows: InsightRow[]): string {
  ...
  const body = rows.map((r) => [
    r.date,
    r.accountId,
    r.effectiveStatus === "ACTIVE" ? "正常" : "暂停",
    r.impressions,
    r.clicks,
    r.reach,
    r.ctr.toFixed(2),
...
function renderBudgetCell(record: InsightRow) {
  ...
  if (record.budgetMode === "ABO") {
    return <span style={{ fontSize: 12, color: "#8c8c8c" }}>ABO · {bidText}</span>;
  }
  const sourceText = "CBO";
...
type CampaignInsightPageProps = {
  ...
  onOpenCreateEditor?: (scope: "campaign", row: InsightRow) => void;
};

const CampaignInsightPage: React.FC<CampaignInsightPageProps> = ({
  forcedAccountId,
  forcedAccountIds,
  hideAccountFilter,
  hideOrgFilters,
  ...
  const [filterAccountId, setFilterAccountId] = useState<string | undefined>(initAccountId);
  const [filterName, setFilterName] = useState<string | undefined>(undefined);
  const [filterCampaignId, setFilterCampaignId] = useState<string | undefined>(searchParams.get("campaignId") || undefined);
  const [filterTeamId, setFilterTeamId] = useState<string | undefined>(searchParams.get("teamId") || undefined);
  const [filterGroupId, setFilterGroupId] = useState<string | undefined>(searchParams.get("groupId") || undefined);
  ...
  const hasInitProject = !!searchParams.get("groupId");
  const [queryParams, setQueryParams] = useState<Record<string, any>>({
    ...
    teamId: hasInitProject ? undefined : (searchParams.get("teamId") || undefined),
  ...
  const handleStatusChange = async (record: InsightRow, checked: boolean) => {
    ...
    try {
      const res = await updateCampaignRemoteStatus(record.id, checked ? "ACTIVE" : "PAUSED");
      hideLoading();
      if (res?.success) {
  ...
  const handleBatchStatusChange = async (status: "ACTIVE" | "PAUSED") => {
    ...
    message.loading({ content: `正在批量${opText}...`, key: "campaign-batch-status", duration: 0 });
    try {
      let successCount = 0;
      for (const row of selectedRows) {
        const res = await updateCampaignRemoteStatus(row.id, status);
        if (res?.success) successCount += 1;
      }
      const failureCount = selectedRows.length - successCount;
      if (failureCount === 0) {
        message.success({ content: `批量${opText}成功，共 ${successCount} 条`, key: "campaign-batch-status" });
      } else if (successCount > 0) {
        message.warning({ content: `批量操作完成：成功 ${successCount} 条，失败 ${failureCount} 条`, key: "campaign-batch-status" });
      } else {
        message.error({ content: "批量操作失败", key: "campaign-batch-status" });
      }
      setSelectedRowKeys([]);
      setSelectedRows([]);
      actionRef.current?.reload();
    } catch (_e) {
      message.error({ content: `批量${opText}失败，请稍后重试`, key: "campaign-batch-status" });
    } finally {
  ...
  const handleSyncConfirm = async () => {
    ...
    try {
      if (syncTarget === null) {
        await syncAllCampaigns(startDate, endDate, syncMode);
      } else if (Array.isArray(syncTarget)) {
        for (const row of syncTarget as InsightRow[]) {
          await syncCampaignById(row.id, startDate, endDate, syncMode);
        }
      } else {
        await syncCampaignById(
          (syncTarget as InsightRow).id,
          startDate,
          endDate,
          syncMode
  ...
  const handleExport = async () => {
    ...
    link.href = url;
    link.download = `campaign_insights_${dayjs().format("YYYYMMDD")}.csv`;
    link.click();
        ...
        return (
          <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
              ...
              <Typography.Text copyable={{ text: record.campaignId }} style={{ fontSize: 12, lineHeight: 1, color: "#8c8c8c" }} />
            </div>
        ...
        <Space size={8}>
          {onOpenCreateEditor ? (
            <Button
              ...
              size="small"
              icon={<EditOutlined />}
              onClick={() => onOpenCreateEditor("campaign", record)}
            >
              编辑
            </Button>
          ) : null}
  ...
  return (
    <>
      <ProTable<InsightRow>
        ...
        rowSelection={{
          ...
          onChange: (_keys, rows) => {
            ...
            const idList = rows.map((r) => String(r.campaignId));
        ...
        tableAlertOptionRender={() => (
          <Space>
            <AdsConsoleAuthButton size="small" code="ads:campaign:sync" icon={<SyncOutlined />} onClick={handleBatchSync} loading={batchActionLoading}>
              {batchActionLoading ? "操作中..." : "批量同步"}
            </AdsConsoleAuthButton>
            <AdsConsoleAuthButton size="small" code="ads:campaign:edit" onClick={() => handleBatchStatusChange("ACTIVE")} loading={batchActionLoading}>
              批量开启
            </AdsConsoleAuthButton>
            <AdsConsoleAuthButton size="small" code="ads:campaign:edit" onClick={() => handleBatchStatusChange("PAUSED")} loading={batchActionLoading}>
              批量关停
            </AdsConsoleAuthButton>
          </Space>
        )}
        ...
        {detailRecord && (
          <Descriptions bordered column={2} size="small">
            ...
            <Descriptions.Item label="状态">
              {detailRecord.effectiveStatus === "ACTIVE" ? "正常" : "暂停"}
            </Descriptions.Item>
      ...
      <ModalForm
        ...
        onFinish={async (values) => {
          const res = editRecord
            ? await updateCampaign({ id: editRecord.id, ...values } as any)
        ...
        <Row gutter={16}>
          <Col span={12}>
            <ProFormText
              name="name"
              label="活动名称"
              rules={[{ required: !editRecord, message: "请输入活动名称" }]}
              placeholder="请输入活动名称"
            />
          </Col>
          <Col span={12}>
      ...
      <Modal
        ...
        afterOpenChange={(open) => {
          if (open && countryRecord) {
            ...
            getCountryInsights({
              objectId: countryRecord.campaignId,
              objectType: "campaign",
        ...
        title={
          <span>
            ...
            <span style={{ marginLeft: 8, fontSize: 13, fontWeight: 400, color: "#8c8c8c" }}>
              活动 {creativeModalRecord?.campaignId}
            </span>
          </span>
        }
      ...
      >
        {creativeModalRecord && (
          <CreativeManagePage
            key={`creative-modal-${creativeModalRecord.campaignId}`}
            forcedAccountId={creativeModalRecord.accountId}
            forcedCampaignIds={[creativeModalRecord.campaignId]}
            hideAccountFilter
      ...
      <AdsConsoleSyncHistoryChart
        ...
        title={`同步历史 · 活动 ${syncChartRecord?.accountId ?? ""}`}
      />
    </>
  );
};

export default CampaignInsightPage;
