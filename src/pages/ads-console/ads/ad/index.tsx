...
import {
  ...
  syncAllAds,
  updateAd,
  updateAdRemoteStatus,
} from "@/services/ads-console/ad";
...
function isAdActiveStatus(status?: string): boolean {
  const value = String(status ?? "").toUpperCase();
  return value === "1" || value === "ACTIVE";
}

function adStatusText(status?: string): string {
  return isAdActiveStatus(status) ? "正常" : "暂停";
}
...
function renderBudgetCell(record: InsightRow) {
  ...
  const remainingMinor = parseMoney(record.budgetMode === "ABO" ? record.adsetBudgetRemaining : record.campaignBudgetRemaining);
...
const AdInsightPage: React.FC<AdInsightPageProps> = ({
  forcedAccountId,
  forcedAccountIds,
  forcedCampaignIds,
  forcedAdsetIds,
  ...
  const [campaignIdOptions, setCampaignIdOptions] = useState<
    { label: string; value: string }[]
  >([]);
  const [adIdOptions, setAdIdOptions] = useState<
    { label: string; value: string }[]
  >([]);
  ...
  const [filterAdsetId, setFilterAdsetId] = useState<string | undefined>(initAdsetId);
  const [filterAdId, setFilterAdId] = useState<string | undefined>(undefined);
  ...
  useEffect(() => {
    setSelectedRowKeys([]);
    setSelectedRows([]);
  }, [forcedCampaignIds, forcedAdsetIds]);
  ...
  const handleStatusChange = async (record: InsightRow, checked: boolean) => {
    ...
    try {
      const res = await updateAdRemoteStatus(
        record.id,
  ...
  const handleBatchStatusChange = async (status: "ACTIVE" | "PAUSED") => {
    ...
    message.loading({ content: `正在批量${opText}...`, key: "ad-batch-status", duration: 0 });
    try {
      let successCount = 0;
      for (const row of selectedRows) {
        const res = await updateAdRemoteStatus(row.id, status);
        if (res?.success) successCount += 1;
      }
      const failureCount = selectedRows.length - successCount;
      if (failureCount === 0) {
        message.success({ content: `批量${opText}成功，共 ${successCount} 条`, key: "ad-batch-status" });
      } else if (successCount > 0) {
        message.warning({ content: `批量操作完成：成功 ${successCount} 条，失败 ${failureCount} 条`, key: "ad-batch-status" });
      } else {
        message.error({ content: "批量操作失败", key: "ad-batch-status" });
      }
      setSelectedRowKeys([]);
      setSelectedRows([]);
      actionRef.current?.reload();
    } catch (_e) {
      message.error({ content: `批量${opText}失败，请稍后重试`, key: "ad-batch-status" });
    } finally {
  ...
  const handleSyncConfirm = async () => {
    ...
    try {
      if (syncTarget === null) {
        await syncAllAds(startDate, endDate, syncMode);
      } else if (Array.isArray(syncTarget)) {
        for (const row of syncTarget as InsightRow[]) {
          await syncAdById(row.id, startDate, endDate, syncMode);
        }
      } else {
        await syncAdById((syncTarget as InsightRow).id, startDate, endDate, syncMode);
      }
    } catch {
      hasError = true;
  ...
  const handleExport = async () => {
    ...
    link.href = url;
    link.download = `ad_insights_${dayjs().format("YYYYMMDD")}.csv`;
    link.click();
  ...
  const columns: ProColumns<InsightRow>[] = [
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        return (
          <Tooltip title={isArchived ? "已归档，无法修改状态" : switchLoadingIds.has(record.id) ? "开关操作中，请稍候..." : undefined}>
            <Switch
              checked={isAdActiveStatus(record.status)}
              checkedChildren="开"
              unCheckedChildren="关"
              disabled={isArchived || switchLoadingIds.has(record.id)}
        ...
        return (
          <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
              ...
              <Typography.Text copyable={{ text: record.adId }} style={{ fontSize: 12, lineHeight: 1, color: "#8c8c8c" }} />
            </div>
    ...
    },
    {
      ...
      valueEnum: {
        ...
        ADSET_PAUSED: { text: "广告组暂停", status: "Warning" },
      ...
      render: (_, record) => {
        ...
        const map: Record<string, { text: string; color: string }> = {
          ...
          ADSET_PAUSED: { text: "广告组暂停", color: "orange" },
        ...
        <Space size={8}>
          {onOpenCreateEditor ? (
            <Button
              ...
              size="small"
              icon={<EditOutlined />}
              onClick={() => onOpenCreateEditor("ad", record)}
            >
              编辑
            </Button>
          ) : null}
  ...
  return (
    <>
      <ProTable<InsightRow>
        ...
        rowSelection={{
          ...
          onChange: (keys, rows) => {
            ...
            onSelectionChange?.(rows.map((row) => String(row.adId)).filter(Boolean));
          },
        ...
        tableAlertOptionRender={() => (
          <Space>
            <AdsConsoleAuthButton size="small" code="ads:ad:sync" icon={<SyncOutlined />} onClick={handleBatchSync} loading={batchActionLoading}>
              {batchActionLoading ? "操作中..." : "批量同步"}
            </AdsConsoleAuthButton>
            <AdsConsoleAuthButton size="small" code="ads:ad:edit" onClick={() => handleBatchStatusChange("ACTIVE")} loading={batchActionLoading}>
              批量开启
            </AdsConsoleAuthButton>
            <AdsConsoleAuthButton size="small" code="ads:ad:edit" onClick={() => handleBatchStatusChange("PAUSED")} loading={batchActionLoading}>
              批量关停
            </AdsConsoleAuthButton>
          </Space>
        )}
        request={async (params, sort) => {
          ...
          const res = await getAdInsights({
            ...
            adsetId: hasForcedAdsetScope ? undefined : adsetId,
          ...
          const records = Array.isArray(res?.data?.records)
            ? ((res?.data?.records ?? []) as unknown as InsightRow[])
            : [];
      ...
      >
        {detailRecord && (
          <Descriptions bordered column={2} size="small">
            ...
            <Descriptions.Item label="广告组ID" span={2}>
              {detailRecord.adsetId}
            </Descriptions.Item>
            <Descriptions.Item label="广告ID" span={2}>
              {detailRecord.adId}
            </Descriptions.Item>
            <Descriptions.Item label="状态">
              {adStatusText(detailRecord.status)}
            </Descriptions.Item>
      ...
      <ModalForm
        ...
        onFinish={async (values) => {
          const res = editRecord
            ? await updateAd({ id: editRecord.id, ...values } as any)
        ...
        >
          <div>
            <ProFormText
              name="name"
              label="广告名称"
              rules={[{ required: !editRecord, message: "请输入广告名称" }]}
              placeholder="请输入广告名称"
            />
          </div>
          <div>
      ...
      >
        {creativeModalRecord && (
          <CreativeManagePage
            key={`creative-modal-${creativeModalRecord.adId}`}
            forcedAccountId={creativeModalRecord.accountId}
            forcedAdIds={[creativeModalRecord.adId]}
            hideAccountFilter
            hideHierarchyFilters
          />
        )}
      ...
      <AdsConsoleSyncHistoryChart
        ...
        title={`同步历史 · 广告 ${syncChartRecord?.accountId ?? ""}`}
      />
    </>
  );
};

export default AdInsightPage;
