...
import { DownloadOutlined, FilterOutlined, SettingOutlined, SearchOutlined } from '@ant-design/icons';
import { ProTable, ActionType, ProColumns } from '@ant-design/pro-components';
import { useDashboard } from '../DashboardContext';
import { getTrafficHourly, getTrafficDaily, getTrafficMonthly, getTrafficSyncJobs } from '@/services/traffic-platform/api';
...
const UsageDataTabs: React.FC = () => {
  ...
  const hourlyColumns: ProColumns<any>[] = [
    { title: '时间', dataIndex: 'statTime', width: 160 },
    { title: '平台', dataIndex: 'platformCode', width: 100 },
    { title: '账号', dataIndex: 'accountName', width: 140 },
    { title: '地区', key: 'geo', width: 120, render: (_, r) => r.region || r.geo || '-' },
    { title: '流量 (GB)', key: 'gb', width: 120, render: (_, r) => toGb(r) },
  ];
  ...
  const monthlyColumns: ProColumns<any>[] = [
    { title: '月份', dataIndex: 'statMonth', width: 120 },
    { title: '平台', dataIndex: 'platformCode', width: 100 },
    { title: '账号', dataIndex: 'accountName', width: 140 },
    { title: '流量 (GB)', key: 'gb', width: 120, render: (_, r) => toGb(r) },
  ];
  ...
  const syncColumns: ProColumns<any>[] = [
    ...
    { title: '状态', dataIndex: 'status', width: 100, render: (v, r) => {
        let color = '#2563EB';
        let bg = '#EFF6FF';
        if (v === 'success') { color = '#10B981'; bg = '#ECFDF5'; }
        else if (v === 'failed') { color = '#EF4444'; bg = '#FEF2F2'; }
        return (
          <span 
            onClick={() => v === 'failed' && setSyncErrorData(r)}
            style={{ 
              color, backgroundColor: bg, padding: '2px 8px', borderRadius: '4px', fontSize: '12px',
              cursor: v === 'failed' ? 'pointer' : 'default'
            }}
          >
            {String(v)}
          </span>
  ...
  const getColumns = () => {
    switch (activeTab) {
      case 'hourly': return hourlyColumns;
      case 'monthly': return monthlyColumns;
      case 'sync': return syncColumns;
      default: return dailyColumns;
    }
  };
