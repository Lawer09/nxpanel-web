...
import { getDashboardIncomeSummary } from '@/services/report/api';
import {
  getRetention,
  getActiveUsers,
...
const toIncomeMetrics = (period?: API.DashboardIncomeSummaryPeriod): IncomeMetrics => {
  if (!period) return EMPTY_INCOME_METRICS;

  return {
    income: safeNum(period.income),
    revenue: safeNum(period.revenue),
    expense: safeNum(period.expense),
  };
};
...
const DashboardPage: React.FC = () => {
  ...
  const { data: incomeSummaryRes, loading: incomeLoading } = useRequest(
    () => getDashboardIncomeSummary({ appId }),
    { refreshDeps: [appId] },
  );
  ...
  const incomeSummary = ((incomeSummaryRes as any)?.data ?? incomeSummaryRes) as
    | API.DashboardIncomeSummaryResult
    | undefined;
  const todayIncomeMetrics = useMemo(
    () => toIncomeMetrics(incomeSummary?.today),
    [incomeSummary],
  );
  const monthIncomeMetrics = useMemo(
    () => toIncomeMetrics(incomeSummary?.month),
    [incomeSummary],
  );
  ...
  const appIdOptions = useMemo(() => {
    ...
    return raw
      .map((item) => {
        const value = String(typeof item === 'string' ? item : item?.appId || item?.value || '').trim();
